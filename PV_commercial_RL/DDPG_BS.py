...
Project: Battery schedule by DDPG
@author: yazeli
Version: 2
"""
...
class SaveOnBestTrainingRewardCallback(BaseCallback):
    ...
    def __init__(self, check_freq: int, log_dir: str, season: int, verbose=1):
        ...
        self.season = season
        self.save_path = os.path.join(log_dir, 'best_model_'+str(['summer','winter'][self.season]))
...
class BatteryEnv(Env):
  ...
  def __init__(self, m, train):
      ...
      self.m = m # month 0-11
      self.train = train
      self.action_space = Box(low=-N_b*Qmax,high=N_b*Qmax,shape=(1,),dtype=np.float32)
  ...
  def step(self, action):
      ...
      if c_nxt > s[self.m,self.current_step]:
          q = (s[self.m,self.current_step]-c)/Gamma_e
      if c_nxt < 0:
          q = -c*Gamma_e
      ...
      if self.train:
          ...
          if qmax_day < qld: # peak load
              ...
              qopt = max(qmax_day-qld, -N_b*Qmax) # discharge to 0(summer) or qmax(winter) to shift load
              phi = 20
...
model = DDPG(CustomPolicy, train, nb_train_steps = 24,
             normalize_returns = True, normalize_observations = True,
             verbose = 0, tensorboard_log = log_dir, 
             param_noise = AdaptiveParamNoiseSpec(),  param_noise_adaption_interval = T[m,0])

model.learn(total_timesteps = time_steps, callback = callback)
#results_plotter.plot_results([log_dir], time_steps, results_plotter.X_TIMESTEPS, "DDPG BS")
#plt.show()
#%%
# model = DDPG.load("best_model_winter", env=train, policy=CustomPolicy, tensorboard_log=log_dir, param_noise = AdaptiveParamNoiseSpec())
# model.learn(total_timesteps = time_steps, callback = callback)
#%% Testing
m = 20
test = BatteryEnv(m,train=False)
model = DDPG.load("best_model_summer_from06", env=test, policy=CustomPolicy, tensorboard_log=log_dir, param_noise = AdaptiveParamNoiseSpec())
obs = test.reset()
dones = False
...
while not dones:
    ...
    obsers[:,k] = obs
    C_ddpg = C_ddpg + rewards
    k = k+1
    test.render()
result = {"q": actions, "soc": obsers[2,:]}
#savemat('q_py_19.mat',result)